...
class FpRoute extends NormalRoute
{
    private $client_data;
...
//        var_dump($route);
        if ($count == 2) {
            ...
            $this->client_data->method_name = $route[$count - 1];
            return;
        }
        $this->client_data->method_name = lcfirst($route[$count-1]) ?? null;
        unset($route[0]);
        $this->client_data->controller_name = implode("\\", $route);
//        var_dump($request->server['path_info'], $this->client_data->controller_name, $this->client_data->method_name);
    }
